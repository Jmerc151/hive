    ...
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-s1 border border-s4 rounded-xl w-full max-w-lg shadow-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        {(suggestions.length > 0 || refreshing) && (
          <div className="p-5 border-b border-s4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-medium text-t2">Scout Suggestions</h3>
              <button
            ...
            <div className="space-y-2">
              {suggestions.slice(0, 5).map(s => (
                <button
                  ...
                  className="w-full text-left p-3 rounded-lg border border-s4 hover:border-scout/50 hover:bg-scout/5 transition-all group"
                >
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm">{BOT_TYPES.find(t => t.value === s.type)?.icon || '🤖'}</span>
                    <span className="font-medium text-sm text-t1 group-hover:text-scout">{s.name}</span>
                    <span className="text-[10px] text-t4 ml-auto">{BOT_TYPES.find(t => t.value === s.type)?.label || s.type}</span>
                  </div>
                  <p className="text-xs text-t3 line-clamp-2">{s.description}</p>
                </button>
              ))}
            </div>
          </div>
        ...
        {suggestions.length === 0 && !refreshing && (
          <div className="px-5 pt-4">
            <button
              onClick={handleRefresh}
              className="w-full p-3 rounded-lg border border-dashed border-s4 text-sm text-t3 hover:border-scout/50 hover:text-scout transition-all"
            >
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {/* Bot Name */}
          <div>
            <label className="block text-sm font-medium text-t2 mb-1.5">Bot Name</label>
            <input type="text" value={name} onChange={e => setName(e.target.value)}
              placeholder="e.g. PriceTracker Pro" autoFocus
              className="w-full bg-page border border-s4 rounded-lg px-3 py-2.5 text-sm text-t1 placeholder:text-t4 focus:outline-none focus:ring-2 focus:ring-t1/50 focus:border-t1" />
          </div>

          {/* Bot Type */}
          <div>
            <label className="block text-sm font-medium text-t2 mb-1.5">Type</label>
          ...
          {/* Description */}
          <div>
            <label className="block text-sm font-medium text-t2 mb-1.5">What should it do?</label>
            <textarea value={description} onChange={e => setDescription(e.target.value)}
              placeholder="Describe the features, behavior, and any specific requirements..."
              rows={4}
              className="w-full bg-page border border-s4 rounded-lg px-3 py-2.5 text-sm text-t1 placeholder:text-t4 focus:outline-none focus:ring-2 focus:ring-t1/50 focus:border-t1 resize-none" />
          </div>

          {/* Advanced options */}
          <details className="group">
            <summary className="text-xs text-t3 cursor-pointer hover:text-t2">
              Advanced options (optional)
            </summary>
            <div className="mt-3 space-y-3">
              <div>
                <label className="block text-sm font-medium text-t2 mb-1.5">Target Audience</label>
                <input type="text" value={audience} onChange={e => setAudience(e.target.value)}
                  placeholder="e.g. Small business owners, developers..."
                  className="w-full bg-page border border-s4 rounded-lg px-3 py-2.5 text-sm text-t1 placeholder:text-t4 focus:outline-none focus:ring-2 focus:ring-t1/50 focus:border-t1" />
              </div>
              <div>
                <label className="block text-sm font-medium text-t2 mb-1.5">Monetization Strategy</label>
          ...
          {/* Forge preview */}
          <div className="flex items-start gap-3 p-3 bg-s3 rounded-lg border border-s4">
            <span className="text-2xl">⚒️</span>
            <div>
              <div className="font-medium text-sm" style={{ color: '#3b82f6' }}>Forge</div>
              <div className="text-xs text-t3 mt-0.5">Will generate complete, runnable code with README, package.json, and .env.example</div>
            </div>
          </div>
