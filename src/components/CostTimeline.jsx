...
export default function CostTimeline({ onClose, inline }) {
  ...
  const content = (
    <div className={inline ? "h-full overflow-y-auto" : "bg-page rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto"} style={inline ? {} : { border: '0.5px solid rgba(0,0,0,0.08)' }} onClick={e => e.stopPropagation()}>
      <div className="flex items-center justify-between p-4" style={{ borderBottom: '0.5px solid rgba(0,0,0,0.08)' }}>
        ...
        <div className="flex items-center gap-3">
          ...
          {!inline && <button onClick={onClose} className="text-t3 hover:text-t1 text-xl" aria-label="Close cost timeline">&times;</button>}
        </div>
      </div>
