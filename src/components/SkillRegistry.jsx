      ...
      <div className="bg-s1 border border-s4 rounded-xl w-full max-w-md shadow-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-3">
          ...
          {skills.map(skill => {
            ...
            return (
              <div key={skill.id} className="flex items-center gap-3 p-3 bg-s3 rounded-lg border border-s4">
                <span className="text-lg">{st?.icon || '⚙️'}</span>
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-sm text-t1">{skill.name}</div>
                  {skill.description && <div className="text-xs text-t3 truncate">{skill.description}</div>}
                  <span className="text-[10px] text-t4">{st?.label || skill.type}</span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={async () => { await api.deleteSkill(skill.id); refresh() }}
                    className="text-xs text-danger hover:text-red-300">×</button>
                </div>
              </div>
            )
          })}
          ...
          {skills.length === 0 && !showForm && (
            <div className="text-center text-t4 py-6 text-sm">No skills configured. Add skills to enhance this agent.</div>
          )}

          {/* Add Skill */}
          <button onClick={() => setShowForm(!showForm)}
            className="w-full p-2.5 border border-dashed border-s4 rounded-lg text-sm text-t3 hover:border-t1/50 hover:text-t1 transition-all">
            + Add Skill
          </button>

          {showForm && (
            <form onSubmit={handleCreate} className="space-y-3 p-3 bg-s3 rounded-lg border border-s4">
              <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Skill name"
                className="w-full bg-page border border-s4 rounded-lg px-3 py-2 text-sm text-t1 placeholder:text-t4 focus:outline-none focus:ring-2 focus:ring-t1/50" />
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {SKILL_TYPES.map(st => (
                  <button key={st.value} type="button" onClick={() => setType(st.value)}
                    className={`p-2 rounded-lg border text-xs transition-all ${type === st.value ? 'border-t1 bg-s3 text-t1' : 'border-s4 text-t3'}`}>
                    {st.icon} {st.label}
                  </button>
                ))}
              </div>
              <button type="submit" disabled={!name.trim()}
                className="w-full px-4 py-2 bg-t1 text-white rounded-lg font-medium text-sm hover:bg-t2 transition-colors disabled:opacity-40">
                Add Skill
              </button>
            </form>
          )}
